...
public class UserTest2 {
	...
	public void testPostUser() {
		...
		Response response = UserEndPoints2.createUser(userPaylaod);
	...
	public void testGetUserByName() {
		...
		Response response = UserEndPoints2.readUser(this.userPaylaod.getUsername());
		response.then().log().all();
		//response.statusCode();
		Assert.assertEquals(response.getStatusCode(), 200);
	...
	public void updateUserByName() {
		...
		userPaylaod.setLastName(faker.name().lastName());	
		userPaylaod.setEmail(faker.internet().safeEmailAddress());		
		Response response = UserEndPoints2.updateeUser(this.userPaylaod.getUsername(), userPaylaod);
		response.then().log().body().statusCode(200);
		Assert.assertEquals(response.getStatusCode(), 200);
		
		//Check data after update
		Response responseAfterUpdate = UserEndPoints2.readUser(this.userPaylaod.getUsername());
		response.then().log().all();
	...
	public void testDeleteUserByName() {
		...
		Response response = UserEndPoints2.deleteUser(this.userPaylaod.getUsername());
		Assert.assertEquals(response.getStatusCode(), 200);
